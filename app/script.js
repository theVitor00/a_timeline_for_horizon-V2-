...
const historicalPeriods = [
  ...
  },
  {
    ...
    endYear: 3040,
    color: '#32cd32',
    description: "Aloy's journey to uncover the truth about the Old World",
  },
  {
    id: 'frozen-wilds',
    name: 'Frozen Wilds',
...
function updateActivePeriod(currentYear) {
  ...
  if (currentPeriod) {
    ...
    periodMarkers.forEach((marker) => {
      if (
        marker.querySelector('.period-label').textContent === currentPeriod.name
      ) {
...
function navigateToIndex(newIndex) {
  ...
  currentPosition = activeElementPosition - containerWidth / 2 + eventWidth / 2;
